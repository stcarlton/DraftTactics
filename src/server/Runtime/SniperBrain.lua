...
function SniperBrain.Update(unit, dt)
	if unit.Strategy == Strategy.Strategy.Blitz then
		SniperBrain.UpdateBlitz(unit, dt)
	elseif unit.Strategy == Strategy.Strategy.Stalk then
		SniperBrain.UpdateStalk(unit, dt)
	elseif unit.Strategy == Strategy.Strategy.Defend then
		SniperBrain.UpdateDefend(unit, dt)
	end
end

function SniperBrain.UpdateBlitz(unit, dt)
...
function SniperBrain.UpdateStalk(unit, dt)
	local root = unit.Root
	local pos = root.Position
	local node = unit.AssignedCoverNode
...
function SniperBrain.UpdateDefend(unit, dt)
